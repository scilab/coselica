...
function [x,y,typ]=CMPS_CutTorque(job,arg1,arg2)
x=[];y=[];typ=[];
select job
  case 'plot' then
    standard_draw(arg1,%f,_CMPS_CutTorque_dp);
  case 'getinputs' then
    [x,y,typ]=_CMPS_CutTorque_ip(arg1);
  case 'getoutputs' then
    [x,y,typ]=_CMPS_CutTorque_op(arg1);
  case 'getorigin' then
  ...
  case 'define' then
    ...
    model.out=[1;1];
    mo=modelica();
      mo.model='Coselica.Mechanics.Planar.Sensors.CutTorque';
      mo.inputs=['frame_a'];
      mo.outputs=['frame_b','torque'];
    ...
    gr_i=[
          ...
          'e.line_style=1;';
          'if orient then';
          '  xstringb(orig(1)+sz(1)*0.25,orig(2)+sz(2)*-0.1,""torque"",sz(1)*0.75,sz(2)*0.2,""fill"");';
          'else';
          '  xstringb(orig(1)+sz(1)*(1-0.25-0.75),orig(2)+sz(2)*-0.1,""torque"",sz(1)*0.75,sz(2)*0.2,""fill"");';
          'end';
          'e=gce();';
          'e.visible=""on"";';
          'e.foreground=color(0,0,0);';
