...
function [x,y,typ]=CMPS_RelAngularAccelerat(job,arg1,arg2)
x=[];y=[];typ=[];
select job
  case 'plot' then
    standard_draw(arg1,%f,_CMPS_Angle_dp);
  case 'getinputs' then
    [x,y,typ]=_CMPS_Angle_ip(arg1);
  case 'getoutputs' then
    [x,y,typ]=_CMPS_Angle_op(arg1);
  case 'getorigin' then
  ...
  case 'define' then
    ...
    model.out=[1;1];
    mo=modelica();
      mo.model='Coselica.Mechanics.Planar.Sensors.RelAngularAcceleration';
      mo.inputs=['frame_a'];
      mo.outputs=['frame_b','z_rel'];
    ...
    gr_i=[
          ...
          'e.line_style=1;';
          'if orient then';
          '  xstringb(orig(1)+sz(1)*0.6,orig(2)+sz(2)*0,""z_rel"",sz(1)*0.7,sz(2)*0.15,""fill"");';
          'else';
          '  xstringb(orig(1)+sz(1)*(1-0.6-0.7),orig(2)+sz(2)*0,""z_rel"",sz(1)*0.7,sz(2)*0.15,""fill"");';
          'end';
          'e=gce();';
          'e.visible=""on"";';
          'e.foreground=color(0,0,0);';
