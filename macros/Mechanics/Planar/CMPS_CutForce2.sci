...
function [x,y,typ]=CMPS_CutForce2(job,arg1,arg2)
x=[];y=[];typ=[];
select job
  case 'plot' then
    standard_draw(arg1,%f,_CMPS_CutForce2_dp);
  case 'getinputs' then
    [x,y,typ]=_CMPS_CutForce2_ip(arg1);
  case 'getoutputs' then
    [x,y,typ]=_CMPS_CutForce2_op(arg1);
  case 'getorigin' then
  ...
  case 'define' then
    ...
    model.out=[1;2];
    mo=modelica();
      mo.model='Coselica.Mechanics.Planar.Sensors.CutForce2';
      mo.inputs=['frame_a','frame_resolve'];
      mo.outputs=['frame_b','force'];
    ...
    gr_i=[
          ...
          'e.line_style=1;';
          'if orient then';
          '  xstringb(orig(1)+sz(1)*0.125,orig(2)+sz(2)*-0.1,""force"",sz(1)*0.75,sz(2)*0.2,""fill"");';
          'else';
          '  xstringb(orig(1)+sz(1)*(1-0.125-0.75),orig(2)+sz(2)*-0.1,""force"",sz(1)*0.75,sz(2)*0.2,""fill"");';
          'end';
          'e=gce();';
          'e.visible=""on"";';
          'e.foreground=color(0,0,0);';
          ...
          'e.fill_mode=""off"";';
          'xpoly(xx+ww*[0.1;0.1],yy+hh*[0;0.5]);';
          'e=gce();';
          'e.visible=""on"";';
          ...
          'e.thickness=0.25;';
          'e.line_style=1;';
          'xpoly(xx+ww*[0.9;0.9],yy+hh*[0.01;0.495]);';
          'e=gce();';
          'e.visible=""on"";';
          ...
          'e.line_style=1;';
          'if orient then';
          '  xstringb(orig(1)+sz(1)*0.9,orig(2)+sz(2)*0.04,""resolve"",sz(1)*0.6,sz(2)*0.125,""fill"");';
          'else';
          '  xstringb(orig(1)+sz(1)*(1-0.9-0.6),orig(2)+sz(2)*0.04,""resolve"",sz(1)*0.6,sz(2)*0.125,""fill"");';
          'end';
          'e=gce();';
          'e.visible=""on"";';
          'e.foreground=color(0,0,0);';
