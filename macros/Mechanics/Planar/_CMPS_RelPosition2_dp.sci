...
function [] = _CMPS_RelPosition2_dp( o )
  ...
  if orient then
    ...
    xfpoly(resolve(:,1)+ones(4,1)*(orig(1)+sz(1)/5),..
    ...
    xfpoly(resolve(:,1)+ones(4,1)*(orig(1)+4*sz(1)/5),..
          resolve(:,2)+ones(4,1)*(orig(2)-yf/40),1);
    e=gce();
    e.foreground=color(0,0,0);
    e.background=color(192,192,192);
  ...
  // output (bottom)
  xfpoly(out(:,1)+ones(3,1)*(orig(1)+sz(1)/2),..
         out(:,2)+ones(3,1)*(orig(2)-yf/15),1);
  e=gce();
