...
select job
  ...
  case 'define' then
    ...
    w_rel_start=0;
    z_rel_start=0;
    model.sim='CMPJ_FreeMotion';
    model.blocktype='c';
    model.dep_ut=[%t %f];
    mo=modelica();
      mo.model='CMPJ_FreeMotion';
      mo.inputs=['frame_a'];
      mo.outputs=['frame_b'];
    ...
    exprs=[sci2exp(initType);sci2exp(r_rel_a_start);sci2exp(v_rel_a_start);sci2exp(a_rel_a_start);sci2exp(phi_rel_start);sci2exp(w_rel_start);sci2exp(z_rel_start)];
    gr_i=[...
          'if orient then';...
          '  xx=orig(1);yy=orig(2);';...
          '  ww=sz(1);hh=sz(2);';...
