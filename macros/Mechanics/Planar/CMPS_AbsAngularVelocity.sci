...
function [x,y,typ]=CMPS_AbsAngularVelocity(job,arg1,arg2)
x=[];y=[];typ=[];
select job
  case 'plot' then
  ...
  case 'define' then
    ...
    model.out=[1];
    mo=modelica();
      mo.model='Coselica.Mechanics.Planar.Sensors.AbsAngularVelocity';
      mo.inputs=['frame_a'];
      mo.outputs=['y'];
    ...
    gr_i=[
          ...
          'e.line_style=1;';
          'if orient then';
          '  xstringb(orig(1)+sz(1)*0.9,orig(2)+sz(2)*0.19,""w"",sz(1)*0.17,sz(2)*0.17,""fill"");';
          'else';
          '  xstringb(orig(1)+sz(1)*(1-0.9-0.17),orig(2)+sz(2)*0.19,""w"",sz(1)*0.17,sz(2)*0.17,""fill"");';
          'end';
          'e=gce();';
          'e.visible=""on"";';
          'e.foreground=color(0,0,0);';
