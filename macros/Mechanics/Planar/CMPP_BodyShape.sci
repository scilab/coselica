...
select job
  ...
  case 'set' then
    ...
    while %t do
      [ok,r,r_CM,m,I,initType,r_0_start,v_0_start,a_0_start,phi_start,w_start,z_start,exprs]=...
        getvalue(['';'CMPP_BodyShape';'';'Rigid body with mass, inertia tensor and two frame connectors (6 potential states)';''],...
  ...
  case 'define' then
    ...
    w_start=0;
    z_start=0;
    model.sim='CMPP_BodyShape';
    model.blocktype='c';
    model.dep_ut=[%t %f];
    mo=modelica();
      mo.model='CMPP_BodyShape';
      mo.inputs=['frame_a'];
      mo.outputs=['frame_b'];
    ...
    exprs=[sci2exp(r);sci2exp(r_CM);sci2exp(m);sci2exp(I);sci2exp(initType);sci2exp(r_0_start);sci2exp(v_0_start);sci2exp(a_0_start);sci2exp(phi_start);sci2exp(w_start);sci2exp(z_start)];
    gr_i=[...
          'if orient then';...
          '  xx=orig(1);yy=orig(2);';...
          '  ww=sz(1);hh=sz(2);';...
