...
function [x,y,typ]=CMPS_RelPosition(job,arg1,arg2)
x=[];y=[];typ=[];
select job
  case 'plot' then
  ...
  case 'define' then
    ...
    model.out=[1;2];
    mo=modelica();
      mo.model='Coselica.Mechanics.Planar.Sensors.RelPosition';
      mo.inputs=['frame_a'];
    ...
    x.graphics.out_style=['fillColor=white;strokeColor=blue','shape=triangle;fillColor=white;strokeColor=blue'];
end
endfunction
