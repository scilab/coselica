...
function [x,y,typ]=CMTS_PowerSensor(job,arg1,arg2)
x=[];y=[];typ=[];
select job
  case 'plot' then
    standard_draw(arg1,%f,_CMTS_ForceSensor_dp);
  case 'getinputs' then
    [x,y,typ]=_CMTS_ForceSensor_ip(arg1);
  case 'getoutputs' then
    [x,y,typ]=_CMTS_ForceSensor_op(arg1);
  case 'getorigin' then
  ...
  case 'define' then
    ...
    model.out=[1;1];
    mo=modelica();
      mo.model='Coselica.Mechanics.Translational.Sensors.PowerSensor';
      mo.inputs=['flange_a'];
      mo.outputs=['flange_b','power'];
    ...
    gr_i=[
          ...
          'e.fill_mode=""off"";';
          'if orient then';
          '  xstringb(orig(1)+sz(1)*0.2,orig(2)+sz(2)*-0.1,""power"",sz(1)*0.4,sz(2)*0.25,""fill"");';
          'else';
          '  xstringb(orig(1)+sz(1)*(1-0.2-0.4),orig(2)+sz(2)*-0.1,""power"",sz(1)*0.4,sz(2)*0.25,""fill"");';
          'end';
          'e=gce();';
          'e.visible=""on"";';
          'e.foreground=color(0,0,0);';
