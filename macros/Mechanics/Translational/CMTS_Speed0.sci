...
function [x,y,typ]=CMTS_Speed0(job,arg1,arg2)
x=[];y=[];typ=[];
select job
  case 'plot' then
  ...
  case 'define' then
    ...
    gr_i=[
          ...
          'end';
          'if orient then';
          '  xstringb(orig(1)+sz(1)*-0.2,orig(2)+sz(2)*0.1,""v_ref"",sz(1)*0.5,sz(2)*0.17,""fill"");';
          'else';
          '  xstringb(orig(1)+sz(1)*(1--0.2-0.5),orig(2)+sz(2)*0.1,""v_ref"",sz(1)*0.5,sz(2)*0.17,""fill"");';
          'end';
          'e=gce();';
          'e.visible=""on"";';
