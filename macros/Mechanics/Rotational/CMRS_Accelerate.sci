...
function [x,y,typ]=CMRS_Accelerate(job,arg1,arg2)
x=[];y=[];typ=[];
select job
  case 'plot' then
  ...
  case 'define' then
    ...
    gr_i=[
          ...
          'e.line_style=1;';
          'if orient then';
          '  xstringb(orig(1)+sz(1)*-0.22,orig(2)+sz(2)*0.07,""a_ref"",sz(1)*0.49,sz(2)*0.18,""fill"");';
          'else';
          '  xstringb(orig(1)+sz(1)*(1--0.22-0.49),orig(2)+sz(2)*0.07,""a_ref"",sz(1)*0.49,sz(2)*0.18,""fill"");';
          'end';
          'e=gce();';
          'e.visible=""on"";';
          'e.foreground=color(0,0,0);';
