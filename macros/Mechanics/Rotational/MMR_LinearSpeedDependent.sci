...
function [x,y,typ]=MMR_LinearSpeedDependent(job,arg1,arg2)
x=[];y=[];typ=[];
select job
  case 'plot' then
  ...
  case 'set' then
    ...
    while %t do
      [ok,tau_nominal,TorqueDirection,w_nominal,exprs]=...
        getvalue(['';'MMR_LinearSpeedDependent';'';'Linear dependency of torque versus speed';''],...
  ...
  case 'define' then
    ...
    TorqueDirection=1;
    w_nominal=1;
    model.sim='MMR_LinearSpeedDependent';
    model.blocktype='c';
    model.dep_ut=[%t %f];
    mo=modelica();
      mo.model='MMR_LinearSpeedDependent';
      mo.inputs=[];
