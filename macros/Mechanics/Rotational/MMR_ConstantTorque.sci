...
function [x,y,typ]=MMR_ConstantTorque(job,arg1,arg2)
x=[];y=[];typ=[];
select job
  case 'plot' then
  ...
  case 'set' then
    ...
    while %t do
      [ok,tau_constant,exprs]=...
        getvalue(['';'MMR_ConstantTorque';'';'Constant torque, not dependent on speed';''],...
  ...
  case 'define' then
    model=scicos_model();
    tau_constant=1;
    model.sim='MMR_ConstantTorque';
    model.blocktype='c';
    model.dep_ut=[%t %f];
    mo=modelica();
      mo.model='MMR_ConstantTorque';
      mo.inputs=[];
