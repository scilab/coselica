...
function [x,y,typ]=CMRS_Speed(job,arg1,arg2)
x=[];y=[];typ=[];
select job
  case 'plot' then
  ...
  case 'define' then
    ...
    gr_i=[
          ...
          'e.line_style=1;';
          'if orient then';
          '  xstringb(orig(1)+sz(1)*-0.29,orig(2)+sz(2)*0.11,""w_ref"",sz(1)*0.52,sz(2)*0.17,""fill"");';
          'else';
          '  xstringb(orig(1)+sz(1)*(1--0.29-0.52),orig(2)+sz(2)*0.11,""w_ref"",sz(1)*0.52,sz(2)*0.17,""fill"");';
          'end';
          'e=gce();';
          'e.visible=""on"";';
          'e.foreground=color(0,0,0);';
