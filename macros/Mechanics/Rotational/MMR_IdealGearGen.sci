...
function [x,y,typ]=MMR_IdealGearGen(job,arg1,arg2)
    x=[];y=[];typ=[];
    select job
     case 'set' then
      x=arg1;
      ...
      while %t do
          ...
          if ok then
              if fixedframe==1 then
                  model.equations.model = 'Modelica.Mechanics.Rotational.IdealGear0';
                  model.out = [1];
                  model.equations.outputs = ['flange_b'];
                  graphics.out_implicit=['I'];
                  graphics.out_style=[RotOutputStyle()];
                  graphics.style = "MMR_IdealGear0";
              else
                  model.equations.model = 'Modelica.Mechanics.Rotational.IdealGear';
                  model.equations.outputs=['flange_b','bearing'];
                  model.out = [1 ; 1]
                  graphics.out_implicit=['I','I'];
                  graphics.out_style=[RotOutputStyle(), RotOutputStyle()];
                  graphics.style = "MMR_IdealGear";
              end
     ...
     case 'define' then
      ...
      x.graphics.style = "MMR_IdealGear0";
    end
endfunction
