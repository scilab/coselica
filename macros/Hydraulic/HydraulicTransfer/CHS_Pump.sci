...
function [x,y,typ]=CHS_Pump(job,arg1,arg2)
x=[];y=[];typ=[];
select job
  case 'plot' then
...
//    [x,y]=standard_origin(arg1);
  case 'set' then
    ...
    while %t do
      [ok,Qnom,Pnom,exprs]=...
        getvalue(['';'CHS_Pump';'';'Volumetric pump';''],...
        [' Qnom [GPM] : nominal debit';' Pnom [psi] : nominal pressure'],...
        list('vec',1,'vec',1),exprs);
      if ~ok then break, end
      model.equations.parameters(2)=list(Qnom,Pnom)
  ...
  case 'define' then
    ...
    model.dep_ut=[%t %f];
    mo=modelica();
      mo.model='Coselica.Hydraulic.Sources.Pump';
      mo.inputs=['rotor'];
      mo.outputs=['Outlet'];
      mo.parameters=list(['Qnom','Pnom'],...
                         list(Qnom,Pnom),...
                         [0,0]);
    model.equations=mo;
    model.in=ones(size(mo.inputs,'*'),1);
    model.out=ones(size(mo.outputs,'*'),1);
    exprs=string([Qnom;Pnom]);
