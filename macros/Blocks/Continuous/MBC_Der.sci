...
function [x,y,typ]=MBC_Der(job,arg1,arg2)
x=[];y=[];typ=[];
select job
  case 'plot' then
  ...
  case 'define' then
    ...
    model.out=[1];
    mo=modelica();
      mo.model='Modelica.Blocks.Continuous.Der';
      mo.inputs=['u'];
      mo.outputs=['y'];
    ...
    gr_i=[
          ...
          'e.fill_mode=""off"";';
          'if orient then';
          '  xstringb(orig(1)+sz(1)*0.1,orig(2)+sz(2)*0.09,""der()"",sz(1)*0.8,sz(2)*0.79,""fill"");';
          'else';
          '  xstringb(orig(1)+sz(1)*(1-0.1-0.8),orig(2)+sz(2)*0.09,""der()"",sz(1)*0.8,sz(2)*0.79,""fill"");';
          'end';
          'e=gce();';
          'e.visible=""on"";';
          'e.foreground=color(0,0,0);';
