...
function [x,y,typ]=MBC_PI(job,arg1,arg2)
x=[];y=[];typ=[];
select job
  case 'plot' then
  ...
  case 'set' then
    ...
    while %t do
      [ok,k,T,exprs]=..
        getvalue(['';'MBC_PI';'';'Proportional-Integral controller';''],..
        [' k [-] : Gain';' T [s] : Time Constant (T>0 required)'],..
  ...
  case 'define' then
    ...
    model.out=[1];
    mo=modelica();
      mo.model='Modelica.Blocks.Continuous.PI';
      mo.inputs=['u'];
      mo.outputs=['y'];
    ...
    gr_i=[
          ...
          'e.thickness=0.25;';
          'e.line_style=1;';
          'xpoly(xx+ww*[0.1;0.1;0.8],yy+hh*[0.1;0.4;0.9]);';
          'e=gce();';
          'e.visible=""on"";';
          ...
          'e.line_style=1;';
          'if orient then';
          '  xstringb(orig(1)+sz(1)*0.5,orig(2)+sz(2)*0.22,""PI"",sz(1)*0.3,sz(2)*0.31,""fill"");';
          'else';
          '  xstringb(orig(1)+sz(1)*(1-0.5-0.3),orig(2)+sz(2)*0.22,""PI"",sz(1)*0.3,sz(2)*0.31,""fill"");';
          'end';
          'e=gce();';
          'e.visible=""on"";';
          'e.foreground=color(0,0,0);';
