...
function [x,y,typ]=MBM_Max(job,arg1,arg2)
x=[];y=[];typ=[];
select job
  case 'plot' then
  ...
  case 'define' then
    ...
    model.out=[1];
    mo=modelica();
      mo.model='Modelica.Blocks.Math.Max';
      mo.inputs=['u1','u2'];
      mo.outputs=['y'];
    ...
    gr_i=[
          ...
          'e.fill_mode=""off"";';
          'if orient then';
          '  xstringb(orig(1)+sz(1)*0.05,orig(2)+sz(2)*0.32,""max()"",sz(1)*0.9,sz(2)*0.36,""fill"");';
          'else';
          '  xstringb(orig(1)+sz(1)*(1-0.05-0.9),orig(2)+sz(2)*0.32,""max()"",sz(1)*0.9,sz(2)*0.36,""fill"");';
          'end';
          'e=gce();';
          'e.visible=""on"";';
          'e.foreground=color(0,0,0);';
