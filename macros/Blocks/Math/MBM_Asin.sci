...
function [x,y,typ]=MBM_Asin(job,arg1,arg2)
x=[];y=[];typ=[];
select job
  case 'plot' then
  ...
  case 'define' then
    ...
    model.out=[1];
    mo=modelica();
      mo.model='Modelica.Blocks.Math.Asin';
      mo.inputs=['u'];
      mo.outputs=['y'];
    ...
    gr_i=[
          ...
          'e.thickness=0.25;';
          'e.line_style=1;';
          'xpoly(xx+ww*[0.1;0.104;0.112;0.132;0.1685;0.2325;0.349;0.687;0.7875;0.8435;0.876;0.888;0.9],yy+hh*[0.1;0.136;0.1625;0.203;0.251;0.3135;0.4015;0.624;0.704;0.7635;0.811;0.8375;0.9]);';
          'e=gce();';
          'e.visible=""on"";';
          'e.foreground=color(0,0,0);';
          'e.thickness=0.25;';
          ...
          'e.line_style=1;';
          'if orient then';
          '  xstringb(orig(1)+sz(1)*0.06,orig(2)+sz(2)*0.65,""asin"",sz(1)*0.36,sz(2)*0.24,""fill"");';
          'else';
          '  xstringb(orig(1)+sz(1)*(1-0.06-0.36),orig(2)+sz(2)*0.65,""asin"",sz(1)*0.36,sz(2)*0.24,""fill"");';
          'end';
          'e=gce();';
          'e.visible=""on"";';
          'e.foreground=color(0,0,0);';
