...
function [x,y,typ]=MBM_Sin(job,arg1,arg2)
x=[];y=[];typ=[];
select job
  case 'plot' then
  ...
  case 'define' then
    ...
    model.out=[1];
    mo=modelica();
      mo.model='Modelica.Blocks.Math.Sin';
      mo.inputs=['u'];
      mo.outputs=['y'];
    ...
    gr_i=[
          ...
          'e.line_style=1;';
          'if orient then';
          '  xstringb(orig(1)+sz(1)*0.56,orig(2)+sz(2)*0.68,""sin"",sz(1)*0.36,sz(2)*0.24,""fill"");';
          'else';
          '  xstringb(orig(1)+sz(1)*(1-0.56-0.36),orig(2)+sz(2)*0.68,""sin"",sz(1)*0.36,sz(2)*0.24,""fill"");';
          'end';
          'e=gce();';
          'e.visible=""on"";';
          'e.foreground=color(0,0,0);';
