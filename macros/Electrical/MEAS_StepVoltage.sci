...
function [x,y,typ]=MEAS_StepVoltage(job,arg1,arg2)
    x=[];y=[];typ=[];
    select job
     case 'set' then
      x=arg1;
      ...
      model=arg1.model;
      while %t do
          [ok,V,offset,startTime,exprs]=...
              getvalue(['';'MEAS_StepVoltage';'';'Step voltage source';''],...
                       [' V [V] : Height of step';' offset [V] : Voltage offset';' startTime [s] : Time offset'],...
     ...
     case 'define' then
      ...
      exprs=[sci2exp(V);sci2exp(offset);sci2exp(startTime)];
      gr_i=[];
