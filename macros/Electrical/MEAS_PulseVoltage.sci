...
function [x,y,typ]=MEAS_PulseVoltage(job,arg1,arg2)
x=[];y=[];typ=[];
select job
  case 'plot' then
  ...
  case 'set' then
    ...
    model=arg1.model;
    while %t do
      [ok,V,width,period,offset,startTime,exprs]=...
        getvalue(['';'MEAS_PulseVoltage';'';'Pulse voltage source';''],...
        [' V [V] : Amplitude of pulse';' width [-] : Width of pulse in % of period';' period [s] : Time for one period';' offset [V] : Voltage offset';' startTime [s] : Time offset'],...
  ...
  case 'define' then
    ...
    exprs=[sci2exp(V);sci2exp(width);sci2exp(period);sci2exp(offset);sci2exp(startTime)];
    gr_i=[];
