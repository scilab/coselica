...
function [x,y,typ]=MEAS_HeatingPNP(job,arg1,arg2)
    x=[];y=[];typ=[];
    select job
     case 'set' then
      x=arg1;
      ...
      while %t do
          [ok,Bf,Br,Is,Vak,Tauf,Taur,Ccs,Cje,Cjc,Phie,Me,Phic,Mc,Gbc,Gbe,EMin,EMax,Tnom,XTI,XTB,EG,NF,NR,exprs]=...
              getvalue(['';'MEAS_HeatingPNP';'';'Simple PNP BJT according to Ebers-Moll with heating port';''],...
                       [' Bf [-] : Forward beta';' Br [-] : Reverse beta';' Is [A] : Transport saturation current';' Vak [1/V] : Early voltage (inverse), 1/Volt';' Tauf [s] : Ideal forward transit time';' Taur [s] : Ideal reverse transit time';' Ccs [F] : Collector-substrat(ground) cap.';' Cje [F] : Base-emitter zero bias depletion cap.';' Cjc [F] : Base-coll. zero bias depletion cap.';' Phie [V] : Base-emitter diffusion voltage';' Me [-] : Base-emitter gradation exponent';' Phic [V] : Base-collector diffusion voltage';' Mc [-] : Base-collector gradation exponent';' Gbc [S] : Base-collector conductance';' Gbe [S] : Base-emitter conductance';' EMin [-] : if x < EMin, the exp(x) function is linearized';' EMax [-] : if x > EMax, the exp(x) function is linearized';' Tnom (K): Parameter measurement temperature';' XTI : Temperature exponent for effect on Is';' XTB : Forward and reverse beta temperature exponent';' EG : Energy gap for temperature effect on Is';' NF : Forward current emission coefficient';' NR : Reverse current emission coefficient'],...
     ...
     case 'define' then
      ...
      model.dep_ut=[%t %f];
      mo=modelica();
      mo.model='Modelica.Electrical.Analog.Semiconductors.HeatingPNP';
