...
function [x,y,typ]=MEAS_HeatingPMOS(job,arg1,arg2)
    x=[];y=[];typ=[];
    select job
     case 'set' then
      x=arg1;
      ...
      while %t do
          [ok,W,L,Beta,Vt,K2,K5,dW,dL,RDS,Tnom,kvt,kk2,exprs]=...
              getvalue(['';'MEAS_HeatingPMOS';'';'Simple PMOS Transistor with heating port';''],...
                       [' W [m] : Width';' L [m] : Length';' Beta [A/(V*V)] : Transconductance parameter';' Vt [V] : Zero bias threshold voltage';' K2 [-] : Bulk threshold parameter';' K5 [-] : Reduction of pinch-off region';' dW [m] : Narrowing of channel';' dL [m] : Shortening of channel';' RDS [Ohm] : Drain-Source-Resistance';' Tnom (K) : Parameter measurement temperature';' kvt : fitting parameter for Vt';' kk2 : fitting parameter for K22'],...
     ...
     case 'define' then
      ...
      model.dep_ut=[%t %f];
      mo=modelica();
      mo.model='Modelica.Electrical.Analog.Semiconductors.HeatingPMOS';
